...
async def raid(ctx, area, levels):
    ...
    if not areaexist:
        ...
        return
    await ctx.message.delete()
    bot.boards['raids'].append({"channel_id": message.channel.id, "message_id": message.id, "title": locale['raids'], "area": area, "timezone": config['timezone'], "wait": 15, "levels": level_list, "ex": False})
