...
async def execute(config, pool, query):
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(query)
    return await cur.fetchall()

async def get_oldest_mon_date(config, pool, use_alt_table=False):
    if use_alt_table:
    ...
    oldest_mon_date = await execute(config, pool, query)
    for var in oldest_mon_date:
        oldest_mon_date = var[0]

    return oldest_mon_date

async def get_shiny_count(mon_id, area, starttime, endtime, config, pool, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    ...
    shiny_count = await execute(config, pool, query)
    for var in shiny_count:
        shiny_count = var[0]

    return shiny_count
    
async def get_shiny_total(mon_id, area, starttime, endtime, config, pool, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    shiny_total = await execute(config, pool, query)
    for var in shiny_total:
        shiny_total = var[0]

    return shiny_total

async def get_scan_numbers(mon_id, area, starttime, endtime, config, pool, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    hundo_count = await execute(config, pool, query)

    return hundo_count

async def get_big_numbers(mon_id, area, starttime, endtime, config, pool, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    big_numbers = await execute(config, pool, query)

    return big_numbers

async def get_active_raids(config, pool, area, level_list, tz_offset, ex=False):
    levels = "("
    ...
    raids = await execute(config, pool, query)

    return raids

async def get_active_quests(config, pool, area):
    if config['db_scan_schema'] == "mad":
    ...
    active_quests = await execute(config, pool, query)

    return active_quests

async def get_gym_stats(config, pool, area):
    if config['db_scan_schema'] == "mad":
    ...
    gym_stats = await execute(config, pool, query)

    return gym_stats

async def statboard_mon_active(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_mon_active = await execute(config, pool, query)

    return statboard_mon_active

async def statboard_mon_today(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_mon_today = await execute(config, pool, query)

    return statboard_mon_today

async def statboard_hundos_active(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_hundos_active = await execute(config, pool, query)

    return statboard_hundos_active

async def statboard_hundos_today(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_hundos_today = await execute(config, pool, query)

    return statboard_hundos_today

async def statboard_iv0_active(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_iv0_active = await execute(config, pool, query)

    return statboard_iv0_active

async def statboard_iv0_today(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_iv0_today = await execute(config, pool, query)

    return statboard_iv0_today

async def statboard_scanned_active(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_scanned_active = await execute(config, pool, query)

    return statboard_scanned_active

async def statboard_scanned_today(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_scanned_today = await execute(config, pool, query)

    return statboard_scanned_today

async def statboard_total_iv_active(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_total_iv_active = await execute(config, pool, query)

    return statboard_total_iv_active

async def statboard_total_iv_today(config, pool, area, use_alt_table=False):
    if use_alt_table:
        table = config['alt_pokemon_table']
    else:
        table = 'pokemon'
    ...
    statboard_total_iv_today = await execute(config, pool, query)

    return statboard_total_iv_today

async def statboard_gym_amount(config, pool, area):
    if config['db_scan_schema'] == "mad":
    ...
    statboard_gym_amount = await execute(config, pool, query)

    return statboard_gym_amount

async def statboard_gym_teams(config, pool, area):
    if config['db_scan_schema'] == "mad":
    ...
    statboard_gym_teams = await execute(config, pool, query)

    return statboard_gym_teams

async def statboard_raid_active(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def statboard_egg_active(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def statboard_stop_amount(config, pool, area):
    if config['db_scan_schema'] == "mad":
    ...
    statboard_stop_amount = await execute(config, pool, query)

    return statboard_stop_amount

async def statboard_lure_active(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def statboard_grunt_active(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def statboard_leader_active(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def statboard_quest_active(config, pool, area):
    if config['db_scan_schema'] == "mad":
    ...
    statboard_quest_active = await execute(config, pool, query)

    return statboard_quest_active
    
async def get_data(config, pool, area, mon_id):
    if config['db_scan_schema'] == "mad":
    ...
    quests = await execute(config, pool, query)
    
    return quests

async def get_alt_data(config, pool, area, mon_id):
    if config['db_scan_schema'] == "mad":
    ...
    quests2 = await execute(config, pool, query)
    
    return quests2

async def get_dataitem(config, pool, area, item_id):
    if config['db_scan_schema'] == "mad":
...
async def get_alt_dataitem(config, pool, area, item_id):
    if config['db_scan_schema'] == "mad":
...
async def get_datamega(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def get_alt_datamega(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def get_datastar(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def get_alt_datastar(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def get_datak(config, pool, area):
    if config['db_scan_schema'] == "mad":
...
async def get_datacoin(config, pool, area):
    if config['db_scan_schema'] == "mad":
