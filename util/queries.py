...
async def get_active_raids(config, area, level_list, ex=False):
    levels = "("
...
async def get_gym_stats(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_gym_stats.execute(f"select count(gym.gym_id), IFNULL(SUM(team_id = 0), 0), IFNULL(SUM(team_id = 1), 0), IFNULL(SUM(team_id=2), 0), IFNULL(SUM(team_id=3), 0), IFNULL(SUM(is_ex_raid_eligible = 1), 0), IFNULL(SUM(end >= utc_timestamp()), 0) from gym left join raid on gym.gym_id = raid.gym_id where " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_mon_active(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_mon_active.execute(f"select count(pokemon_id) from {table} where disappear_time > utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_mon_today(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_mon_today.execute(f"select count(pokemon_id) from {table} where CONVERT_TZ({table}.disappear_time,'+00:00','{config.timezone}') > curdate() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_hundos_active(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_hundos_active.execute(f"select count(pokemon_id) from {table} where individual_attack = 15 AND individual_defense = 15 AND individual_stamina = 15 AND disappear_time > utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_hundos_today(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_hundos_today.execute(f"select count(pokemon_id) from {table} where individual_attack = 15 AND individual_defense = 15 AND individual_stamina = 15 AND CONVERT_TZ({table}.disappear_time,'+00:00','{config.timezone}') > curdate() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_iv0_active(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_iv0_active.execute(f"select count(pokemon_id) from {table} where individual_attack = 0 AND individual_defense = 0 AND individual_stamina = 0 AND disappear_time > utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_iv0_today(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_iv0_today.execute(f"select count(pokemon_id) from {table} where individual_attack = 0 AND individual_defense = 0 AND individual_stamina = 0 AND CONVERT_TZ({table}.disappear_time,'+00:00','{config.timezone}') > curdate() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_scanned_active(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_scanned_active.execute(f"select count(pokemon_id) from {table} where individual_attack is not NULL and disappear_time > utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_scanned_today(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_scanned_today.execute(f"select count(pokemon_id) from {table} where individual_attack is not NULL AND CONVERT_TZ({table}.disappear_time,'+00:00','{config.timezone}') > curdate() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_total_iv_active(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_total_iv_active.execute(f"select ifnull(sum((individual_attack + individual_defense + individual_stamina)/45*100),0) from {table} where individual_attack is not NULL and disappear_time > utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_total_iv_today(config, area, use_alt_table=False):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_total_iv_today.execute(f"select ifnull(sum((individual_attack + individual_defense + individual_stamina)/45*100),0) from {table} where individual_attack is not NULL AND CONVERT_TZ({table}.disappear_time,'+00:00','{config.timezone}') > curdate() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_gym_amount(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_gym_amount.execute(f"select count(gym_id) from gym where " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_gym_teams(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_gym_teams.execute(f"select ifnull(sum(team_id = 0),0), ifnull(sum(team_id = 1),0), ifnull(sum(team_id=2),0), ifnull(sum(team_id=3),0) from gym where " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_raid_active(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_raid_active.execute(f"select count(raid.gym_id), ifnull(sum(level = 1), 0), ifnull(sum(level = 2),0), ifnull(sum(level = 3),0), ifnull(sum(level = 4),0), ifnull(sum(level = 5),0) from gym left join raid on gym.gym_id = raid.gym_id where end >= utc_timestamp() and start <= utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_egg_active(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_egg_active.execute(f"select count(raid.gym_id), ifnull(sum(level = 1), 0), ifnull(sum(level = 2),0), ifnull(sum(level = 3),0), ifnull(sum(level = 4),0), ifnull(sum(level = 5),0) from gym left join raid on gym.gym_id = raid.gym_id where start > utc_timestamp() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_stop_amount(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_stop_amount.execute(f"select count(pokestop_id) from pokestop where " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_lure_active(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_lure_active.execute(f"select count(pokestop_id), ifnull(sum(active_fort_modifier = 501), 0), ifnull(sum(active_fort_modifier = 502), 0), ifnull(sum(active_fort_modifier = 503), 0), ifnull(sum(active_fort_modifier = 504), 0) from pokestop where lure_expiration > UTC_TIMESTAMP() and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_grunt_active(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_grunt_active.execute(f"select count(pokestop_id) from pokestop where incident_expiration > UTC_TIMESTAMP() AND incident_grunt_type NOT IN (41,42,43,44) and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_leader_active(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_leader_active.execute(f"select count(pokestop_id) from pokestop where incident_expiration > UTC_TIMESTAMP() AND incident_grunt_type >= 41 AND incident_grunt_type <= 44 and " + generics["area"])
    elif config.db_scan_schema == "rdm":
...
async def statboard_quest_active(config, area):
    ...
    if config.db_scan_schema == "mad":
        await cursor_statboard_quest_active.execute(f"select count(GUID) from pokestop left join trs_quest on pokestop.pokestop_id = trs_quest.GUID where quest_timestamp > UNIX_TIMESTAMP(CURDATE()) and " + generics["area"])
    elif config.db_scan_schema == "rdm":
