    ...
    async def board_loop(self):
        for board in self.bot.boards['raids']:
            try:
                ...
                text = ""
                raids = await queries.get_active_raids(self.bot.config, self.bot.pool, area[0], board["levels"], board["timezone"], board['ex'])
                if not raids:
        ...
        for board in self.bot.boards['stats']:
            try:
                ...
                if self.bot.config['use_alt_table_for_pokemon']:
                    oldest_mon_date = await queries.get_oldest_mon_date(self.bot.config, self.bot.pool)
                ...
                if "mon_active" in board['type']:
                    mon_active = await queries.statboard_mon_active(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > now_minus_60_min:
                        alt_mon_active = await queries.statboard_mon_active(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "mon_today" in board['type']:
                    mon_today = await queries.statboard_mon_today(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > date_today:
                        alt_mon_today = await queries.statboard_mon_today(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "scanned_active" in board['type']:
                    scanned_active = await queries.statboard_scanned_active(self.bot.config, self.bot.pool, area[0])
                    if not "mon_active" in board['type']:
                        mon_active = await queries.statboard_mon_active(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > now_minus_60_min:
                        alt_scanned_active = await queries.statboard_scanned_active(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "scanned_today" in board['type']:
                    scanned_today = await queries.statboard_scanned_today(self.bot.config, self.bot.pool, area[0])
                    if not "mon_today" in board['type']:
                        mon_today = await queries.statboard_mon_today(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > date_today:
                        alt_scanned_today = await queries.statboard_scanned_today(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "average_iv_active" in board['type']:
                    total_iv_active = await queries.statboard_total_iv_active(self.bot.config, self.bot.pool, area[0])
                    if not "scanned_active" in board['type']: #query for active scanned mons wont run twice if scanned_active in board
                        scanned_active = await queries.statboard_scanned_active(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > now_minus_60_min:
                        alt_total_iv_active = await queries.statboard_total_iv_active(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "average_iv_today" in board['type']:
                    total_iv_today = await queries.statboard_total_iv_today(self.bot.config, self.bot.pool, area[0])
                    if not "scanned_today" in board['type']: #query for today scanned mons wont run twice if scanned_today in board
                        scanned_today = await queries.statboard_scanned_today(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > date_today:
                        alt_total_iv_today = await queries.statboard_total_iv_today(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "hundos_active" in board['type']:
                    hundos_active = await queries.statboard_hundos_active(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > now_minus_60_min:
                        alt_hundos_active = await queries.statboard_hundos_active(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "hundos_today" in board['type']:
                    hundos_today = await queries.statboard_hundos_today(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > date_today:
                        alt_hundos_today = await queries.statboard_hundos_today(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "iv0_active" in board['type']:
                    iv0_active = await queries.statboard_iv0_active(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > now_minus_60_min:
                        alt_iv0_active = await queries.statboard_iv0_active(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "iv0_today" in board['type']:
                    iv0_today = await queries.statboard_iv0_today(self.bot.config, self.bot.pool, area[0])
                    if self.bot.config['use_alt_table_for_pokemon'] and oldest_mon_date > date_today:
                        alt_iv0_today = await queries.statboard_iv0_today(self.bot.config, self.bot.pool, area[0], use_alt_table=True)
                ...
                if "gym_amount" in board['type']:
                    gym_amount = await queries.statboard_gym_amount(self.bot.config, self.bot.pool, area[0])
                    text = f"{text}\n{self.bot.custom_emotes['gym_grey']} **{gym_amount[0][0]:,}** {self.bot.locale['total_gyms']}\n"

                if "gym_teams" in board['type']:
                    gym_teams = await queries.statboard_gym_teams(self.bot.config, self.bot.pool, area[0])
                ...
                if ("raid_active" or "raid_lvl_1_active" or "raid_lvl_2_active" or "raid_lvl_3_active" or "raid_lvl_4_active" or "raid_lvl_5_active" or "raid_lvl_6_active" or "raid_lvl_7_active" or "raid_lvl_8_active" or "raid_lvl_9_active") in board['type']:
                    raid_active = await queries.statboard_raid_active(self.bot.config, self.bot.pool, area[0])
                    ...
                    if "raid_lvl_9_active" in board['type'] and not "egg_lvl_9_active" in board['type']:
                        text = f"{text}6⭐ **{raid_active[0][9]:,}** {self.bot.locale['raids']}\n"
                    if "raid_lvl_8_active" in board['type'] and not "egg_lvl_8_active" in board['type']:
                        text = f"{text}5⭐ **{raid_active[0][8]:,}** {self.bot.locale['raids']}\n"
                    if "raid_lvl_7_active" in board['type'] and not "egg_lvl_7_active" in board['type']:
                        text = f"{text}4⭐ **{raid_active[0][7]:,}** {self.bot.locale['raids']}\n"
                ...
                if ("egg_active" or "egg_lvl_1_active" or "egg_lvl_2_active" or "egg_lvl_3_active" or "egg_lvl_4_active" or "egg_lvl_5_active" or "egg_lvl_6_active" or "egg_lvl_7_active" or "egg_lvl_8_active" or "egg_lvl_9_active") in board['type']:
                    egg_active = await queries.statboard_egg_active(self.bot.config, self.bot.pool, area[0])
                    ...
                    if "raid_lvl_9_active" in board['type']:
                        text = f"{text}6⭐: **{raid_active[0][9]:,}** {self.bot.locale['raids']} | **{egg_active[0][6]:,}** {self.bot.locale['eggs']}\n"
                    elif "egg_lvl_9_active" in board['type']:
                        text = f"{text}{self.bot.custom_emotes['raid_egg_9']} **{egg_active[0][9]:,}** {self.bot.locale['active_eggs']}\n"
                    if "raid_lvl_8_active" in board['type']:
                        text = f"{text}5⭐: **{raid_active[0][8]:,}** {self.bot.locale['raids']} | **{egg_active[0][5]:,}** {self.bot.locale['eggs']}\n"
                    elif "egg_lvl_8_active" in board['type']:
                        text = f"{text}{self.bot.custom_emotes['raid_egg_8']} **{egg_active[0][8]:,}** {self.bot.locale['active_eggs']}\n"
                    if "raid_lvl_7_active" in board['type']:
                        text = f"{text}4⭐: **{raid_active[0][7]:,}** {self.bot.locale['raids']} | **{egg_active[0][4]:,}** {self.bot.locale['eggs']}\n"
                    elif "egg_lvl_7_active" in board['type']:
                        text = f"{text}{self.bot.custom_emotes['raid_egg_7']} **{egg_active[0][7]:,}** {self.bot.locale['active_eggs']}\n"
                ...
                if "stop_amount" in board['type']:
                    stop_amount = await queries.statboard_stop_amount(self.bot.config, self.bot.pool, area[0])
                    text = f"{text}\n{self.bot.custom_emotes['pokestop']} **{stop_amount[0][0]:,}** {self.bot.locale['total_stops']}\n"

                if "quest_active" in board['type']:
                    quest_active = await queries.statboard_quest_active(self.bot.config, self.bot.pool, area[0])
                ...
                if "lure_amount" in board['type']:
                    lure_active = await queries.statboard_lure_active(self.bot.config, self.bot.pool, area[0])
                ...
                if "lure_types" in board['type']:
                    if not "lure_amount" in board['type']: #query wont run twice
                        lure_active = await queries.statboard_gym_teams(self.bot.config, self.bot.pool, area[0]) 
                    text = f"{text}{self.bot.custom_emotes['lure_normal']}**{lure_active[0][1]}**{self.bot.custom_emotes['blank']}{self.bot.custom_emotes['lure_glacial']}**{lure_active[0][2]}**{self.bot.custom_emotes['blank']}{self.bot.custom_emotes['lure_mossy']}**{lure_active[0][3]}**{self.bot.custom_emotes['blank']}{self.bot.custom_emotes['lure_magnetic']}**{lure_active[0][4]}**{self.bot.custom_emotes['blank']}{self.bot.custom_emotes['lure_rainy']}**{lure_active[0][5]}**\n\n"

                if "grunt_active" in board['type']:
                    grunt_active = await queries.statboard_grunt_active(self.bot.config, self.bot.pool, area[0])
                ...
                if "leader_active" in board['type']:
                    leader_active = await queries.statboard_leader_active(self.bot.config, self.bot.pool, area[0])
                    if "grunt_active" in board['type']:
    ...
    async def quest_loop(self):
        for board in self.bot.boards['quests']:
            try:
                ...
                area = get_area(board["area"])
                text = ""
                quests = await queries.get_active_quests(self.bot.config, self.bot.pool, area[0])
                ...
                if (len(board["items"]) + len(board["mons"])) == 1:
                    if len(board["items"]) == 1:
                        embed.set_thumbnail(url=f"{self.bot.config['mon_icon_repo']}reward/item/{board['items'][0]}.png")
                    elif len(board["mons"]) == 1:
                        embed.set_thumbnail(url=f"{self.bot.config['mon_icon_repo']}pokemon/{str(board['mons'][0])}.png")
